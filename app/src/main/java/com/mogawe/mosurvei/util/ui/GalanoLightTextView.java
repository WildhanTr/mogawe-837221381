...
public class GalanoLightTextView extends AppCompatTextView {
    ...
    public GalanoLightTextView(Context context) {
        super(context);
    }

    public GalanoLightTextView(Context context, AttributeSet attrs) {
        super(context, attrs);
        parseAttributes(context, attrs);
    }

    public GalanoLightTextView(Context context, AttributeSet attrs, int defStyleAttr) {
...
//    }
    ...
    private void parseAttributes(Context context, AttributeSet attrs) {
        TypedArray values = context.obtainStyledAttributes(attrs, R.styleable.GalanoLightTextView);
        //The value 0 is a default, but shouldn't ever be used since the attr is an enum
        int textStyle = values.getInt(R.styleable.GalanoLightTextView_android_textStyle, 0);
        switch (textStyle) {
            case Typefaces.NORMAL:
            default:
                //You can instantiate your typeface anywhere, I would suggest as a
                //singleton somewhere to avoid unnecessary copies
                setTypeface(Typefaces.galanoLight(context));
                break;
            case Typefaces.BOLD:
                setTypeface(Typefaces.galanoNormal(context));
                break;
            case Typefaces.ITALIC:
                setTypeface(Typefaces.galanoLightItalic(context));
                break;
