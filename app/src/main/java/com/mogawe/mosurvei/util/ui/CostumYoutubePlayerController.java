...
public class CostumYoutubePlayerController extends AbstractYouTubePlayerListener implements YouTubePlayerFullScreenListener {
    ...
    private YouTubePlayerSeekBar mSeekBar;

    private final YouTubePlayerTracker playerTracker;
    private boolean fullscreen = false;

    public CostumYoutubePlayerController( Context context, View customPlayerUi, YouTubePlayer youTubePlayer, YouTubePlayerView youTubePlayerView) {
...
//        videoCurrentTimeTextView = playerUi.findViewById(R.id.play_time);
//        videoDurationTextView = playerUi.findViewById(R.id.duration_time);
        mSeekBar = playerUi.findViewById(R.id.video_seekbar);

        ImageButton playPauseButton = playerUi.findViewById(R.id.play_pause_video);
        ImageButton enterExitFullscreenButton = playerUi.findViewById(R.id.enter_exit_fullscreen_button);

        youTubePlayer.addListener(mSeekBar);

        mSeekBar.setYoutubePlayerSeekBarListener(new YouTubePlayerSeekBarListener() {
            @Override
...
//        videoCurrentTimeTextView.setText(minutes + " : " + seconds);
    }

    @SuppressLint("SetTextI18n")
    @Override
    public void onVideoDuration(@NonNull YouTubePlayer youTubePlayer, float duration) {
//        videoDurationTextView.setText(minutes + " : " + seconds);
    }
